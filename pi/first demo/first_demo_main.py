...
def load_config(path):
    ...
    seq = [s for s in (normalize_stage(v) for v in cfg.get("sequence", [])) if s]
...
class Outputs:
    ...
    async def apply_stage(self, stage_name, stage_cfg, intensity, bpm):
        ...
        self.send_pd(f"stage {stage_name}")
    ...
    async def start_led(self, profile, intensity):
        ...
        if pattern == "steady":
            ...
            return
        self.led_task = asyncio.create_task(self._led_loop(profile, intensity), name="led-loop")

    async def _led_loop(self, profile, intensity):
    ...
    async def start_motor(self, profile, intensity):
        ...
        if mode == "steady":
            ...
            return
        self.motor_task = asyncio.create_task(self._motor_loop(profile, intensity), name="motor-loop")

    async def _motor_loop(self, profile, intensity):
    ...
    async def start_nebuliser(self, profile, intensity):
        ...
        if mode == "steady":
            ...
            return
        self.neb_task = asyncio.create_task(self._neb_loop(profile, intensity), name="neb-loop")

    async def _neb_loop(self, profile, intensity):
...
class DemoEngine:
    def __init__(self, cfg, log):
        self.cfg = cfg
        self.log = log
        self.protocol = str(cfg.get("protocol", {}).get("type", "dreamdemo")).lower()

        demo_cfg = cfg.get("demo", {})
        self.bpm = to_int(demo_cfg.get("default_bpm", 60), 60)
        self.min_bpm = to_int(demo_cfg.get("min_bpm", 30), 30)
        self.max_bpm = to_int(demo_cfg.get("max_bpm", 180), 180)
        self.intensity = to_float(demo_cfg.get("default_intensity", 1.0), 1.0)
        self.max_intensity = to_float(demo_cfg.get("max_intensity", 1.5), 1.5)

        self.sequence = cfg.get("sequence", ["awake", "light", "deep", "rem"])
        ...
        self.jump_stage = None
        self.control_event = asyncio.Event()
        self.sequence_task = None

        self.outputs = Outputs(cfg, log.getChild("outputs"))

    async def close(self):
        await self.stop("shutdown", abort=True)
        await self.outputs.close()

    async def handle_packet(self, pkt):
        pkt_type = str(pkt.get("t", "")).lower()
        cmd = str(pkt.get("cmd", "")).strip().lower()

        if pkt_type == "sleepstream":
            if pkt.get("bpm") is not None:
                await self.set_bpm(pkt.get("bpm"))
            return

        if pkt_type and pkt_type != self.protocol:
            return

        if cmd in {"start", "run", "trigger"}:
            await self.start("ble_start")
        elif cmd == "stop":
            await self.stop("ble_stop", abort=False)
        elif cmd == "abort":
            await self.stop("ble_abort", abort=True)
        elif cmd in {"next", "advance"}:
            self.next_requested = True
            self.control_event.set()
        elif cmd in {"set_stage", "stage", "jump"}:
            s = normalize_stage(pkt.get("stage", pkt.get("value")))
            if s:
                if self.running:
                    self.jump_stage = s
                    self.control_event.set()
                else:
                    await self.start(f"set_stage_{s}", initial_stage=s)
        elif cmd in {"set_intensity", "intensity"}:
            await self.set_intensity(pkt.get("value", pkt.get("intensity")))
        elif cmd in {"hr", "bpm", "heart_rate"}:
            await self.set_bpm(pkt.get("bpm", pkt.get("value")))
        elif pkt.get("bpm") is not None:
            await self.set_bpm(pkt.get("bpm"))

    async def start(self, reason, initial_stage=None):
        if self.running:
            await self.stop("restart", abort=False)

        idx = 0
        if initial_stage and initial_stage in self.sequence:
            idx = self.sequence.index(initial_stage)
        ...
        self.outputs.send_pd(f"intensity {self.intensity:.3f}")

        self.sequence_task = asyncio.create_task(self.run_sequence(idx), name="demo-sequence")
        self.log.info("Demo started (%s)", reason)

    async def stop(self, reason, abort):
        if not self.running and self.sequence_task is None:
            await self.outputs.off_all()
        ...
        await self.outputs.off_all()
        self.outputs.stop_pd()
        self.log.info("Demo stopped (%s)", reason)

    async def run_sequence(self, idx):
        while self.running and 0 <= idx < len(self.sequence):
            stage = self.sequence[idx]
            self.current_stage = stage
            self.current_index = idx
            await self.apply_stage(stage)

            stage_cfg = self.stages.get(stage, {})
            duration = max(0.1, to_float(stage_cfg.get("duration_s", 20), 20.0))
            ...
            while self.running:
                ...
                if remain <= 0:
                    break
                try:
                    await asyncio.wait_for(self.control_event.wait(), timeout=remain)
                except asyncio.TimeoutError:
                    break
                self.control_event.clear()

                if self.jump_stage:
                    target = self.jump_stage
                    self.jump_stage = None
                    if target in self.sequence:
                        idx = self.sequence.index(target)
                        switched = True
                        break
        ...
        if self.running:
            await self.stop("sequence_complete", abort=False)

    async def apply_stage(self, stage):
        self.log.info("Stage -> %s", stage.upper())
        cfg = self.stages.get(stage, {})
        await self.outputs.apply_stage(stage, cfg, self.intensity, self.bpm)

    async def set_intensity(self, value):
        new_i = clamp(to_float(value, self.intensity), 0.0, self.max_intensity)
        if abs(new_i - self.intensity) < 1e-6:
            return
        self.intensity = new_i
        self.outputs.send_pd(f"intensity {self.intensity:.3f}")
        if self.running and self.current_stage:
            await self.apply_stage(self.current_stage)

    async def set_bpm(self, value):
        new_bpm = int(clamp(to_int(value, self.bpm), self.min_bpm, self.max_bpm))
        if new_bpm == self.bpm:
            return
        self.bpm = new_bpm
        if self.running:
...
class BleReceiver:
    ...
    def drain(self):
        while self.buffer:
            n = self.buffer.find("\n")
            r = self.buffer.find("\r")
            endings = [i for i in (n, r) if i >= 0]
            if endings:
                idx = min(endings)
...
class App:
    def __init__(self, cfg, enable_ble, log):
        ...
        self.demo = DemoEngine(cfg, log.getChild("demo"))
        self.ble = BleReceiver(cfg.get("ble", {}), self.queue, self.stop_event)
